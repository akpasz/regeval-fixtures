...
def main() -> int:
    ...
    if not ak_paths:
        ...
        for c in cases:
            ...
            findings = mutation_check.check(
                corr.known_good_answer, corr.corrupted_answer,
                corr.mutation_class, corr.expected_original,
                corr.expected_corrupted)
            ok &= gate(f"{vc.id}: declared mutation {corr.mutation_class} verified",
